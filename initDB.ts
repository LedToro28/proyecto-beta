...
db.serialize(() => {
  ...
  db.all("PRAGMA table_info(properties)", (err: Error | null, columns: ColumnInfo[]) => {
    if (err) return console.error(err);
    const hasDestacada = columns.some(col => col.name === 'destacada');
    if (!hasDestacada) {
      db.run("ALTER TABLE properties ADD COLUMN destacada INTEGER DEFAULT 0", (err2: Error | null) => {
  ...
  db.get("SELECT id FROM users WHERE username = ?", [adminUsername], async (err: Error | null, row: any) => {
    if (err) {
      console.error(err);
      return db.close();
    }
    if (!row) {
      ...
      db.run("INSERT INTO users (username, email, password_hash, role) VALUES (?, ?, ?, ?)",
        [adminUsername, adminEmail, hash, 'admin'], (err2: Error | null) => {
