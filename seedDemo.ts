  ...
  db.serialize(() => {
    for (const a of agencies) {
      db.run(`INSERT OR IGNORE INTO agencies (name, email, phone, address) VALUES (?, ?, ?, ?)`,
        [a.name, a.email, a.phone, a.address], function(err: Error | null) {
          if (err) return console.error(err.message);
          const agencyId = (this as any).lastID;
          if (agencyId > 0) {
    ...
    setTimeout(() => {
      db.all(`SELECT id FROM agencies ORDER BY id`, (err: Error | null, rows: any[]) => {
        if (err || !rows.length) { console.log('No agencies found'); db.close(); return; }
        const agencyMap: { [key: number]: number } = {};
        rows.forEach((r: any, i: number) => { agencyMap[i + 1] = r.id; });
